...
export const TimestampPicker = ({
  ...
  // Menangani Pilihan Tanggal
  const handleSelectDate = (date: Date) => {
    const newDate = new Date(date)
    if (internalValue) {
      // Pertahankan jam dan menit yang sudah dipilih sebelumnya
      newDate.setHours(internalValue.getHours(), internalValue.getMinutes())
    } else {
      // Default ke 00:00 jika belum ada waktu yang dipilih
  ...
  // Menangani Pilihan Jam
  const handleHourSelect = (hourStr: string) => {
    const hour = parseInt(hourStr, 10)
    const newDate = internalValue ? new Date(internalValue) : new Date()
    newDate.setHours(hour)
    if (!internalValue) newDate.setMinutes(0)

    setInternalValue(newDate)
    onChange?.(newDate)
  }

  // Menangani Pilihan Menit
  const handleMinuteSelect = (minuteStr: string) => {
    const minute = parseInt(minuteStr, 10)
    const newDate = internalValue ? new Date(internalValue) : new Date()
    newDate.setMinutes(minute)
    if (!internalValue) newDate.setHours(0)

    setInternalValue(newDate)
    onChange?.(newDate)
  }

  // Fitur Robust: Menggabungkan "Hari Ini" & "Jam Sekarang" menjadi satu tombol pintasan
  ...
  // ============================
  // Logika Pembuatan UI Waktu
  // ============================
  const hoursList = Array.from({ length: 24 }, (_, i) => i.toString().padStart(2, '0'))
  const minutesList = Array.from({ length: 60 }, (_, i) => i.toString().padStart(2, '0'))

  const currentHour = internalValue ? internalValue.getHours().toString().padStart(2, '0') : ''
  const currentMinute = internalValue ? internalValue.getMinutes().toString().padStart(2, '0') : ''

  // Formatting output
  const formattedDateTime = internalValue
  ...
  const monthYearLabel = new Intl.DateTimeFormat('id-ID', { month: 'long', year: 'numeric' }).format(currentMonth)

  const dropdownContent = isOpen && (
    <div
      ref={dropdownRef}
      style={dropdownStyle}
      className='bg-[hsl(var(--popover))] border border-[hsl(var(--border))] rounded-(--radius) shadow-[0_10px_30px_-10px_hsl(var(--shadow-color)/0.2)] animate-dropdown flex flex-col sm:flex-row select-none overflow-hidden'
    >
      {/* Sisi Kiri: Kalender */}
      <div className='p-3 border-b sm:border-b-0 sm:border-r border-[hsl(var(--border))] w-[288px] shrink-0'>
        ...
        {/* 4. TAMPILAN PANEL RENTANG TAHUN */}
        {view === 'year-range' && (
          <div className='w-full'>
            <div className='flex items-center justify-between mb-4'>
              <button
      ...
      {/* Sisi Kanan: Waktu */}
      <div className='flex flex-col w-full sm:w-48 bg-[hsl(var(--subtle))/0.3]'>
        <div className='flex items-center justify-between px-2 py-2 border-b border-[hsl(var(--border))] bg-[hsl(var(--subtle))]'>
        ...
        <div className='flex h-63 divide-x divide-[hsl(var(--border))]'>
          {/* Kolom Jam */}
