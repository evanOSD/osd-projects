// src/app/(dashboard)/plan-progress/page.tsx
...
import { Plus, Trash2, Edit2, Check, X, ChevronDown, ChevronUp, Calendar, Users, Award, BookOpen, Layers } from 'lucide-react'
...
export default function PlanProgressPage() {
  ...
  if (contextLoading) {
    return (
      <div className='p-6 text-center text-muted-foreground animate-pulse'>
        Menghubungkan ke database dan menginisialisasi skema proyek...
      </div>
    )
  }
  ...
  return (
    <>
      <PageTabs tabs={[{ name: 'Rencana & Progress Proyek', path: '/plan-progress' }]} />

      <div className='p-6 space-y-6 max-w-7xl mx-auto'>
        {/* Project Context Summary Card */}
        <Card className='p-6 bg-linear-to-r from-[hsl(var(--primary))]/5 to-transparent border-l-4 border-[hsl(var(--primary))] flex flex-col md:flex-row justify-between items-start md:items-center gap-4'>
          <div>
            <h1 className='text-2xl font-extrabold text-foreground tracking-tight'>{selectedProject}</h1>
            <p className='text-sm text-muted-foreground mt-1'>
              Bahasa Utama: <strong>{context?.language?.name_in_ethnologue} ({context?.language?.iso_code})</strong>
            </p>
          </div>
          <div className='flex gap-2.5'>
            <Badge variant='success'>Aktif</Badge>
            <Badge variant='info'>Rencana Kerja</Badge>
          </div>
        </Card>
        ...
        <div className='flex flex-wrap gap-2 border-b border-[hsl(var(--border))] pb-2'>
          {subTabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-semibold transition-all cursor-pointer ${
                activeTab === tab.id
