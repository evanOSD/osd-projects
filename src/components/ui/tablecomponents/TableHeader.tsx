...
export function TableHeader<TData, TValue>({ header }: TableHeaderProps<TData, TValue>) {
  ...
  // Deteksi cerdas: Apakah ini kolom pinned paling kanan?
  const isLastLeftPinnedColumn = isPinned === 'left' && header.column.getIsLastColumn('left')
  // Pengecualian: Checkbox (select) tidak boleh diberi batas tegas meskipun ia sendirian
  const shouldHaveBorder = isLastLeftPinnedColumn && header.column.id !== 'select'
  ...
  // LOGIKA PENGGAMBARAN GARIS:
  // Jika ini kolom pinned terakhir -> Gambar garis tebal 2px (w-[2px]) + Bayangan Hitam
  // Jika bukan -> Gambar garis tipis pemisah antar kolom (w-px)
  const rightBorderClass = shouldHaveBorder
    ? 'before:absolute before:top-0 before:bottom-0 before:right-0 before:w-[2px] before:bg-border before:z-10 shadow-[4px_0_12px_-4px_rgba(0,0,0,0.1)]'
    : 'before:absolute before:top-0 before:bottom-0 before:right-0 before:w-px before:bg-muted-foreground/30 before:z-10'

  return (
    <th
      style={style}
      colSpan={header.colSpan}
      className={`px-3 py-3 text-left font-semibold bg-surface align-middle after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-border after:z-10 last:before:hidden ${rightBorderClass}`}
    >
